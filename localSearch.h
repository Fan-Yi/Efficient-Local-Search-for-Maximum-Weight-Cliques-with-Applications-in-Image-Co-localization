...
cout << "add " << v << endl;
		...
		//all vertices in C comes from the clique neighbor set except the first one
...
/////////////////////////////////////////////////////////////////////////
			...
			{
#ifdef clique_hash_mode
				if(best_swap_v == 0 || score[best_swap_v] < 0)//local optimum
				{
...
//cout << "hash bound exceeds" << endl;
				...
				if(best_swap_v == 0 || -vertices[best_remove_v].get_weight() > score[best_swap_v])
				{
					...
					if(best_swap_v == 0)
					{
						int rand_dropped_v = ptr_to_weighted_clique->rand_element();
						drop(rand_dropped_v);
						confChange[rand_dropped_v] = 0;
					}
					else
#endif
					{
...
cout << "answer update times: " << ans_update_times << endl;
	#ifdef clique_hash_mode
cout << "restart times: " << restart_num << endl;
cout << "restart frequency: " << step / restart_num << endl;
	#endif
#endif
		}
		else
